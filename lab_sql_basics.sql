...
select `type`, count(card_id) from card group by `type` order by `type` desc;
...
select date, count(amount) from loan where date < 930907 group by date order by date desc;
...
select date, count(loan_id), duration from loan where date regexp '^9712' group by date, duration order by date, duration asc;
...
round(sum(amount)) as total_amount from trans where account_id = 396 group by `type` order by `type`;
...
round(sum(case when type = 'PRIJEM' then amount end)) - round(sum(case when type = 'VYDAJ' then amount end)) as Difference
